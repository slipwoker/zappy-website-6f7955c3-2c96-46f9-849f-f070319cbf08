document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    contactForm.addEventListener('input', function (e) {
      const field = e.target.closest('input, textarea, select');
      if (field && contactForm.contains(field) && field.classList.contains('input-error')) {
        validateField(field);
      }
    });
  ...
  if (animatableElements.length > 0) {
      ...
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          ...
          setTimeout(function () {
            ...
            el.removeAttribute('data-animate-pending');
          }, parseInt(delay, 10));
    ...
    if ('IntersectionObserver' in window) {
      const scrollObserver = new IntersectionObserver(onIntersect, OBSERVER_OPTIONS);

      animatableElements.forEach(function (el) {
        el.setAttribute('data-animate-pending', 'true');
  ...
  (function applyRTLSupport() {
    ...
    if (isRTL) {
      ...
      const fadeLeftEls = document.querySelectorAll('.fade-left');
      const fadeRightEls = document.querySelectorAll('.fade-right');

      fadeLeftEls.forEach(function (el) {
        el.classList.remove('fade-left');
        el.classList.add('fade-right', 'rtl-swapped');
      });

      fadeRightEls.forEach(function (el) {
        if (!el.classList.contains('rtl-swapped')) {
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '6f7955c3-2c96-46f9-849f-f070319cbf08',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '6f7955c3-2c96-46f9-849f-f070319cbf08';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '6f7955c3-2c96-46f9-849f-f070319cbf08';
